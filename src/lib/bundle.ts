 ...
 */
function createExecutableFromProject(project: any) {
    ...
    for (let i = 0; i < project.length - 1; i++) {
        ...
        const existing: any = contents.find((m) => m.path === mod.path);
 ...
 * @return {Module[]}
 */
function createProjectStructure(mainFile: string) {
    const sorted: any[] = [];
 ...
 * @return {Module[]}
 */
function exploreNodes(node: any, cwd: any) {
